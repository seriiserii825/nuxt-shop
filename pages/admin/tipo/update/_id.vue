...
export default {
  ...
  async asyncData({ $axios, params }) {
    let { tipo } = await $axios.$get(
      process.env.baseUrl + "/api/v1/tipo/" + params.id
    );

    return {
      slug_it: tipo.slug_it,
      slug_en: tipo.slug_en,
      id: tipo.id,
      ruleForm: {
        title_it: tipo.title_it,
        title_en: tipo.title_en
      },
      rules: {
        title_it: [
  ...
  methods: {
    ...
    submitForm() {
      ...
      this.$refs.ruleForm.validate(async valid => {
        if (valid) {
          try {
            const tipo = await this.$axios.$patch(
              process.env.baseUrl + "/api/v1/tipo/" + this.$route.params.id,
              {
                id: this.id,
            ...
            if (tipo.status === 1) {
              ...
              this.$router.push("/admin/tipo");
            } else {
              this.$message.warning(tipo.message);
            }
          } catch (error) {
