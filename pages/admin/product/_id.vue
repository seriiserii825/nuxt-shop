...
export default {
  ...
  methods: {
    ...
    onSubmit(e) {
      ...
      const atrs = JSON.stringify(this.attributes);
      console.log(atrs, "a");
    ...
    getData() {
      ...
      this.$axios.get("/auth/product/" + this.id).then((res) => {
        ...
        this.attributes = attributes.map((attrs) => attrs.id);
        this.server_attributes = attributes;
        console.log(this.server_attributes, "this.server_attributes");
